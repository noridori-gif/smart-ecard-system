...
export async function GET(request:Request){
 ...
 try{const payments=await processAutomaticPaymentAcknowledgements(db);totals.paymentAcknowledgementsSent=payments.sent;totals.paymentAcknowledgementsFailed=payments.failed;totals.paymentAcknowledgementsSkipped+=payments.skipped;}catch(cause){totals.paymentAcknowledgementsFailed+=1;console.info("workflow_processor_payment_acknowledgements",{result:"failed",error:workflowErrorText(cause)});}
