 ...
 for(const event of events??[]){
  ...
  try{
   if(event.event_type==="pledge.reminder.schedule_requested"){
     ...
     const pledgeId=Number(event.payload?.pledge_id);const [{data:pledge},{data:eventRow},{data:settings}]=await Promise.all([db.from("event_pledge_financial_summary").select("id,full_name,normalized_phone,pledged_amount,total_paid,balance,expected_completion_date").eq("id",pledgeId).eq("event_id",event.event_id).maybeSingle(),db.from("events").select("id,title,language").eq("id",event.event_id).maybeSingle(),db.from("event_finance_automation_settings").select("pledge_acknowledgement_mode,reminder_channel").eq("event_id",event.event_id).maybeSingle()]);
