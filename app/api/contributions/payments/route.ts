...
export async function POST(request:Request){
  ...
  const rawToken=generateReceiptToken();const {data,error}=await client.rpc("record_pledge_payment_with_verification",{target_pledge_id:pledgeId,payment_amount:amount,paid_on:body?.date,method,reference:body?.reference??null,payment_provider:body?.provider??null,payment_notes:body?.notes??null,supplied_receipt_token_hash:hashOrganiserToken(rawToken),received_by_name:body?.receivedBy??null});
