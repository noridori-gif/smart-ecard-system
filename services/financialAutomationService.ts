...
export type AutomationSettings = {
  ...
  custom_meeting_invitation_message:string|null;
...
export type ReminderHistoryRow={id:number;pledge_id:number;channel:ReminderChannel;reminder_type:string;recipient_phone:string|null;created_at:string;sent_at:string|null;delivery_status:string;retry_count:number;next_retry_at:string|null;error_message:string|null;event_pledges:{full_name:string}|Array<{full_name:string}>|null};
...
export type Meeting={id:number;event_id:number;title:string;meeting_date:string;meeting_time:string;venue:string;map_url:string|null;note:string|null;public_map_token?:string|null;status:"draft"|"active"|"completed"|"cancelled";created_at:string;updated_at:string};
export type MeetingInvitationPreview={meeting:Meeting;event:{id:number;title:string;event_date:string;language:"sw"|"en"};rows:Array<{pledgeId:number;contributor:string;phone:string|null;paymentStatus:"pledged"|"partial"|"completed"|"cancelled";channel:ReminderChannel;message:string;publicMapUrl:string|null;smsSegments:number;smsEncoding:"GSM-7"|"UCS-2";smsCharacters:number;smsUnits:number;eligible:boolean;skippedReason:string|null;deliveryStatus:string|null;idempotencyKey:string;errorMessage:string|null}>;provider:Record<ReminderChannel,{configured:boolean;message:string}>};
export type ClosingReportPayment=PledgePayment&{recorded_by_name:string};
...
export function exportClosingWorkbook(report:ClosingReport){
  ...
  add("Payments",validPayments.map(p=>({"Receipt Number":p.receipt_number,Amount:p.amount,Currency:p.currency_code??"TZS","Payment Date":p.payment_date,Method:p.payment_method,Reference:p.payment_reference??"",Provider:p.provider??"","Received By":p.received_by??"","Recorded By":p.recorded_by_name})));
  add("Voided Payments",voidedPayments.map(p=>({"Receipt Number":p.receipt_number,Amount:p.amount,Currency:p.currency_code??"TZS","Payment Date":p.payment_date,Method:p.payment_method,Provider:p.provider??"","Received By":p.received_by??"","Recorded By":p.recorded_by_name,"Void Reason":p.void_reason??"","Voided At":p.voided_at})));
...
async function meetingRequest(eventId:number,body:Record<string,unknown>){
  const {data}=await supabase.auth.getSession();if(!data.session?.access_token)throw new Error("Your session has expired.");
  const response=await fetch(`/api/contributions/meetings/${eventId}`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${data.session.access_token}`},body:JSON.stringify(body)});
  const payload=await safeJson(response);if(!response.ok)throw new Error(typeof payload.error==="string"?payload.error:"Meeting invitation request failed.");return payload;
}
export async function getMeetings(eventId:number){const result=await meetingRequest(eventId,{action:"list"});return (result.meetings??[]) as Meeting[]}
export function saveMeeting(eventId:number,input:{meetingId?:number;title:string;meetingDate:string;meetingTime:string;venue:string;mapUrl:string;note:string}){return meetingRequest(eventId,{action:"save",...input}) as Promise<{meeting:Meeting}>}
export function cancelMeeting(eventId:number,meetingId:number){return meetingRequest(eventId,{action:"cancel",meetingId}) as Promise<{meeting:Meeting}>}
export function previewMeetingInvitations(eventId:number,input:{meetingId:number;audience:MeetingAudience;manualPledgeIds:number[];channels:ReminderChannel[]}){return meetingRequest(eventId,{action:"preview",...input}) as Promise<MeetingInvitationPreview>}
export function sendMeetingInvitations(eventId:number,input:{meetingId:number;audience:MeetingAudience;manualPledgeIds:number[];channels:ReminderChannel[]}){return meetingRequest(eventId,{action:"send",confirmed:true,...input}) as Promise<{queued:number;sent:number;failed:number;skipped:number;errors:string[]}>}
