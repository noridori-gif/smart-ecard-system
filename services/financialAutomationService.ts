...
export type DailySummary={date:string;amount:number;transactions:number;contributors:number;totalPledged:number;totalCollected:number;outstanding:number;percentage:number};
...
export function getReminderEligibility(pledges:FinancialPledge[],eventDate:string,settings:AutomationSettings){
  const eventPassed=new Date(`${eventDate}T23:59:59`).getTime()<Date.now();
  return pledges.map((pledge)=>{
    let reason="";if(!settings.reminders_enabled)reason="Reminders disabled";else if(pledge.calculated_status==="cancelled")reason="Cancelled";else if(Number(pledge.balance)<=0)reason="Completed";else if(!/^255[67]\d{8}$/.test(pledge.normalized_phone))reason="Invalid phone";else if(eventPassed&&settings.stop_after_event_date&&!settings.allow_after_event_date)reason="Event has passed";
    return {pledge,eligible:!reason,reason};
  });
}
export async function getClosingReport(eventId:number):Promise<ClosingReport>{
  const [eventResult,pledgeResult,paymentResult,guestResult,invitationResult]=await Promise.all([
  ...
  const error=eventResult.error||pledgeResult.error||paymentResult.error||guestResult.error||invitationResult.error;if(error)throw new Error(error.message);
...
export function dailySummary(report:ClosingReport,date:string):DailySummary{
  ...
  return {date,amount:payments.reduce((s,p)=>s+Number(p.amount),0),transactions:payments.length,contributors:new Set(payments.map(p=>p.pledge_id)).size,totalPledged:report.financial.totalPledged,totalCollected:report.financial.totalCollected,outstanding:report.financial.outstanding,percentage:report.financial.percentage};
}
export function exportClosingWorkbook(report:ClosingReport){
  ...
  add("Summary",[{Metric:"Event",Value:report.event.title},{Metric:"Event Type",Value:report.event.event_type},{Metric:"Event Date",Value:report.event.event_date},{Metric:"Venue",Value:report.event.venue},{Metric:"Total Guests",Value:report.guestStats.totalGuests},{Metric:"Total Invitations",Value:report.guestStats.totalInvitations},{Metric:"Viewed Invitations",Value:report.guestStats.viewed},{Metric:"Accepted RSVP",Value:report.guestStats.accepted},{Metric:"Maybe RSVP",Value:report.guestStats.maybe},{Metric:"Declined RSVP",Value:report.guestStats.declined},{Metric:"Checked In",Value:report.guestStats.checkedIn},{Metric:"Total Pledged",Value:report.financial.totalPledged},{Metric:"Total Collected",Value:report.financial.totalCollected},{Metric:"Outstanding",Value:report.financial.outstanding},{Metric:"Collection Percentage",Value:report.financial.percentage},{Metric:"Generated",Value:new Date().toISOString()}]);
