...
export type Meeting={id:number;event_id:number;title:string;meeting_date:string;meeting_time:string;venue:string;map_url:string|null;note:string|null;status:"draft"|"active"|"completed"|"cancelled";created_at:string;updated_at:string};
export type MeetingInvitationPreview={meeting:Meeting;event:{id:number;title:string;event_date:string;language:"sw"|"en"};rows:Array<{pledgeId:number;contributor:string;phone:string|null;paymentStatus:"pledged"|"partial"|"completed"|"cancelled";channel:ReminderChannel;message:string;smsSegments:number;smsEncoding:"GSM-7"|"UCS-2";smsCharacters:number;smsUnits:number;eligible:boolean;skippedReason:string|null;deliveryStatus:string|null;idempotencyKey:string;errorMessage:string|null}>;provider:Record<ReminderChannel,{configured:boolean;message:string}>};
