...
export type ThankYouPreview={event:{id:number;title:string;language:"sw"|"en"};rows:Array<{pledgeId:number;contributor:string;phone:string|null;pledgedAmount:string;totalPaid:string;balance:string;channel:ReminderChannel;message:string;eligible:boolean;skippedReason:string|null;deliveryStatus:string|null;completionFingerprint:string;idempotencyKey:string}>;completed:number;eligible:number;alreadyThanked:number;missingPhone:number;invalidPhone:number;skipped:number;provider:Record<ReminderChannel,{configured:boolean;message:string}>};
