...
export default function EventPreviewSwitcher({data,invitationPreview}:{data:EventPreviewData;invitationPreview:ReactNode}) {
  ...
  return <section aria-label={t("preview.region")} className="max-h-[calc(100vh-3rem)] overflow-y-auto rounded-[26px] border border-stone-200 bg-[#f4f1eb] p-4 shadow-[0_18px_50px_rgba(28,25,23,.12)]">
    ...
    <div role="tabpanel" className="mt-4 transition-opacity duration-200">
      ...
      {mode==="whatsapp"&&(data.template==="custom"?<p className="rounded-xl bg-white p-4 text-center text-sm text-slate-600">{data.language==="sw"?"Muundo wa mwaliko binafsi unahakikiwa kwenye sehemu ya \"Muundo Binafsi\" hapo juu.":"The custom design is previewed in the Custom Design section above."}</p>:<div><div className="mb-3 flex justify-center gap-1"><button type="button" aria-pressed={whatsAppView==="card"} onClick={()=>setWhatsAppView("card")} className={`rounded-full px-3 py-2 text-xs font-bold ${whatsAppView==="card"?"bg-slate-950 text-white":"bg-white"}`}>{t("preview.cardImage")}</button><button type="button" aria-pressed={whatsAppView==="message"} onClick={()=>setWhatsAppView("message")} className={`rounded-full px-3 py-2 text-xs font-bold ${whatsAppView==="message"?"bg-slate-950 text-white":"bg-white"}`}>{t("preview.messageView")}</button></div>{whatsAppView==="card"?<div className="mx-auto w-[300px] overflow-hidden rounded-xl bg-white shadow-xl" style={{height:totalCardHeight*PREVIEW_SCALE}}><div style={{width:CARD_WIDTH,height:totalCardHeight,transform:`scale(${PREVIEW_SCALE})`,transformOrigin:"top left"}}><PremiumWhatsAppCard data={payload} template={data.template as PremiumWhatsAppTemplate}/></div></div>:<MessagePreview data={data} names={names}/>}</div>)}
