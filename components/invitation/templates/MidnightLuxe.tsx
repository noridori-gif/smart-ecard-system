...
export default function MidnightLuxe({ invitation, heroTitle, displayedMessage, language }: Props) {
  ...
  const t = language === "en" ? { gala: "An evening to remember", intro: "The hosts request your presence", guest: "Reserved especially for", note: "Your invitation", programme: "The evening programme", ceremony: "Ceremony", reception: "Reception", attire: "Evening Attire", countdown: "The night begins in", pass: "Admission", wishes: "Leave a note for the hosts", closing: "Meet us beneath the evening lights" } : { gala: "Usiku wa kukumbukwa", intro: "Wenye event wanaomba heshima ya uwepo wako", guest: "Imehifadhiwa maalumu kwa", note: "Mwaliko wako", programme: "Ratiba ya jioni", ceremony: "Ibada", reception: "Mapokezi / Sherehe", attire: "Mavazi ya Jioni", countdown: "Usiku unaanza baada ya", pass: "Ruhusa ya Kuingia", wishes: "Acha ujumbe kwa wenye event", closing: "Tukutane chini ya mwanga wa jioni" };
  ...
  return (
    <div className="mx-auto w-full max-w-5xl overflow-hidden bg-[color-mix(in_srgb,var(--theme-secondary)_86%,white)] shadow-[0_35px_110px_rgba(2,6,23,.35)]">
      <header className="relative min-h-[720px] overflow-hidden bg-[color-mix(in_srgb,var(--theme-primary)_55%,#05070d)] text-white sm:min-h-[820px]">
        ...
        <div className="absolute inset-x-4 bottom-7 sm:inset-x-10 sm:bottom-10 lg:left-[12%] lg:right-[8%]">
          <div className="max-w-3xl border-l-4 border-[var(--theme-accent)] bg-[color-mix(in_srgb,var(--theme-secondary)_88%,transparent)] px-5 py-7 text-slate-950 shadow-2xl backdrop-blur-md sm:px-9 sm:py-10">
            <p className="text-[9px] font-black uppercase tracking-[0.38em] text-[var(--theme-primary)]">{invitation.event_title}</p>
      ...
      <section className="relative px-5 py-14 sm:px-12 sm:py-20 lg:px-20"><LightLines className="absolute -right-40 top-0 h-72 w-[650px] text-[var(--theme-accent)] opacity-[.09]" /><div className="relative grid gap-12 lg:grid-cols-[.8fr_1.4fr] lg:gap-20"><div><p className="text-[9px] font-bold uppercase tracking-[0.38em] text-[var(--theme-accent)]">{t.intro}</p><p className="mt-5 font-serif text-3xl leading-tight text-[var(--theme-primary)] sm:text-4xl">{t.guest}</p><h2 className="mt-3 break-words font-serif text-4xl leading-tight text-slate-900 sm:text-5xl">{invitation.guest_name}</h2></div><div className="border-t-2 border-[var(--theme-primary)] pt-6"><p className="text-[9px] font-bold uppercase tracking-[0.38em] text-[var(--theme-primary)]">{t.note}</p><p className="mt-5 whitespace-pre-line text-base leading-8 text-slate-700 sm:text-lg sm:leading-9">{displayedMessage}</p></div></div></section>
