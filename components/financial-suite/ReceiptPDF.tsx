...
export default function ReceiptPDF({ receipt, verificationUrl, qrDataUrl }: { receipt: FinanceReceipt; verificationUrl: string; qrDataUrl: string }) {
  const rows = [
    ...
    ...(receipt.provider?[[receipt.payment_method==="bank"?"Bank Name":receipt.payment_method==="mobile_money"?"Mobile Network":"Provider",receipt.provider]]:[]),
    ...(receipt.received_by?[["Payment Collected By",receipt.received_by]]:[]),
    ["Recorded By (Staff)",receipt.recorded_by],
