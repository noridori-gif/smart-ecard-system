...
export default function InvitationEligibilityQueue({
  ...
  const metrics = [
    ["Tayari Kualikwa", ready], ["Wamealikwa Tayari", invited],
    ["Inahitaji Ukaguzi", pledges.filter((pledge) => pledge.guest_eligibility_status === "needs_review").length],
    ["Hajafikia Kiwango", pledges.filter((pledge) => pledge.guest_eligibility_status === "below_minimum").length],
    ["Asilimia ya Mialiko", totalContributors ? `${((invited / totalContributors) * 100).toFixed(1)}%` : "0.0%"],
  ];
  ...
  return <div className="space-y-5">
    <div><h2 className="sep-section-title">Wanaosubiri Mwaliko</h2><p className="sep-secondary mt-1">Tengeneza na tuma mialiko kwa wachangiaji wanaostahiki na wenye wageni waliounganishwa.</p></div>
    ...
    <section className="sep-card overflow-hidden">
      ...
      <div className="hidden overflow-x-auto lg:block"><table className="w-full min-w-[1200px] text-left text-sm"><thead className="bg-stone-50 text-xs uppercase tracking-wide text-slate-500"><tr><th className="p-3"><span className="sr-only">Chagua</span></th>{["Mchangiaji","Simu","Amelipa","Aina ya Kadi","Wageni Wanaoruhusiwa","Mgeni","Hali ya Mwaliko","Ustahiki","Vitendo"].map((heading) => <th key={heading} className="p-3">{heading}</th>)}</tr></thead><tbody className="divide-y divide-stone-200">{visible.map((row) => <tr key={row.pledge.id}><td className="p-3"><input aria-label={`Chagua ${row.pledge.full_name}`} type="checkbox" checked={selected.has(row.pledge.id)} onChange={() => toggle(row.pledge.id)} /></td><td className="p-3 font-bold">{row.pledge.full_name}</td><td className="p-3">{row.pledge.phone || <NoPhoneBadge />}</td><td className="p-3">{formatTzs(row.pledge.total_paid)}</td><td className="p-3 capitalize">{row.pledge.guest_eligibility_status}</td><td className="p-3">{row.guest.allowed_guests}</td><td className="p-3">{row.guest.full_name}</td><td className="p-3"><Status value={row.invitation ? row.invitation.invitation_status || "generated" : "invitation_pending"} /></td><td className="p-3"><Status value="eligible" /></td><td className="p-3"><RowActions row={row} eventId={eventId} /></td></tr>)}</tbody></table></div>
...
function Status({ value }: { value: string }) { const tone = value === "eligible" || value === "sent" ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-800"; const labels: Record<string,string> = { eligible:"Anastahiki", invitation_pending:"Mwaliko Unasubiri", generated:"Mwaliko Umetengenezwa", sent:"Mwaliko Umetumwa", checked_in:"Ameingia" }; return <span className={`rounded-full px-2.5 py-1 text-xs font-bold ${tone}`}>{labels[value] ?? value.replaceAll("_"," ")}</span>; }
function NoPhoneBadge() { return <span className="inline-flex rounded-full bg-red-50 px-2.5 py-1 text-xs font-bold text-red-700">Hakuna namba ya simu</span>; }
