...
export default function InvitationEligibilityQueue({
  ...
  return <div className="space-y-5">
    ...
    <section className="sep-card overflow-hidden">
      ...
      <div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead className="bg-stone-50 text-xs uppercase tracking-wide text-slate-500"><tr><th className="p-3"><span className="sr-only">Chagua</span></th><th className="p-3">Mchangiaji</th><th className="hidden p-3 xl:table-cell">Simu</th><th className="hidden p-3 lg:table-cell">Amelipa</th><th className="hidden p-3 lg:table-cell">Aina ya Kadi</th><th className="hidden p-3 lg:table-cell">Wageni Wanaoruhusiwa</th><th className="p-3">Hali ya Mwaliko</th><th className="hidden p-3 lg:table-cell">Ustahiki</th><th className="p-3">Vitendo</th></tr></thead><tbody className="divide-y divide-stone-200">{visible.map((row) => <tr key={row.pledge.id}><td className="p-3"><input aria-label={`Chagua ${row.pledge.full_name}`} type="checkbox" checked={selected.has(row.pledge.id)} onChange={() => toggle(row.pledge.id)} /></td><td className="min-w-0 p-3"><p className="break-words font-bold text-slate-950">{row.pledge.full_name}</p><p className="mt-0.5 text-xs text-slate-500">{row.pledge.phone || <NoPhoneBadge />}</p><p className="mt-0.5 truncate text-xs text-slate-400">{row.guest.full_name}</p></td><td className="hidden p-3 xl:table-cell">{row.pledge.phone || <NoPhoneBadge />}</td><td className="hidden p-3 tabular-nums lg:table-cell">{formatTzs(row.pledge.total_paid)}</td><td className="hidden p-3 capitalize lg:table-cell">{row.pledge.guest_eligibility_status}</td><td className="hidden p-3 lg:table-cell">{row.guest.allowed_guests}</td><td className="p-3"><Status value={row.invitation ? row.invitation.invitation_status || "generated" : "invitation_pending"} /></td><td className="hidden p-3 lg:table-cell"><Status value="eligible" /></td><td className="p-3"><RowActions row={row} eventId={eventId} /></td></tr>)}</tbody></table></div>
