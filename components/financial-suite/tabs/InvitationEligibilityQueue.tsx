...
export default function InvitationEligibilityQueue({
  ...
  return <div className="space-y-5">
    ...
    {previewOpen && <Dialog titleId="queue-preview-title" onClose={() => setPreviewOpen(false)} className="max-w-4xl"><div className="flex justify-between gap-3"><div><h2 id="queue-preview-title" className="text-xl font-bold">Invitation Preview</h2><p className="text-sm text-slate-600">{eventTitle} · {template} · {language.toUpperCase()}</p></div><Button variant="ghost" onClick={() => setPreviewOpen(false)}>Close</Button></div><div className="mt-4 overflow-x-auto"><table className="w-full min-w-[760px] text-left text-sm"><thead><tr>{["Guest Name","Template","Language","Card Type","Allowed Guests","Phone","Invitation Link","QR Preview"].map((heading) => <th key={heading} className="p-2">{heading}</th>)}</tr></thead><tbody className="divide-y">{selectedRows.map((row) => <tr key={row.pledge.id}><td className="p-2 font-bold">{row.guest.full_name}</td><td className="p-2">{template || "Missing template"}</td><td className="p-2 uppercase">{language}</td><td className="p-2 capitalize">{row.pledge.guest_eligibility_status}</td><td className="p-2">{row.guest.allowed_guests}</td><td className="p-2">{row.guest.phone ?? "No phone"}</td><td className="p-2">{row.invitation ? <Link className="text-emerald-700 underline" href={`/invite/${row.invitation.invitation_token}`} target="_blank">Open link</Link> : "Generated after confirmation"}</td><td className="p-2"><Link className="text-emerald-700 underline" href={`/events/${eventId}/guests/${row.guest.id}/qr`} target="_blank">Open QR</Link></td></tr>)}</tbody></table></div><Button className="mt-5 w-full" loading={busy === "generate"} disabled={!template || selectedRows.some((row) => Boolean(row.invitation))} onClick={() => void generate()}>Generate Invitations</Button></Dialog>}
  </div>;
}
