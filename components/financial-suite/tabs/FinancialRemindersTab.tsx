...
export default function FinancialRemindersTab({ eventId, eventDate, deadline, pledges }: { eventId: number; eventDate: string; deadline: string | null; pledges: FinancialPledge[] }) {
  ...
  async function sendDailyTest(){if(!dailyPreview||!settings)return;try{setBusy(true);setError("");const result=await sendDailySummaryNow(eventId,dailyPreview.summary.date,dailyChannels);setNotice(`Daily Summary test: ${result.sent} sent, ${result.failed} failed, ${result.skipped} skipped.`);if(result.failed>0&&result.errors.length)setError(result.errors.join(" "));setDailyConfirmed(false);await load()}catch(err){setError(err instanceof Error?err.message:"Daily Summary test failed.")}finally{setBusy(false)}}
