...
export default function FinancialRemindersTab({ eventId, eventDate, deadline, pledges }: { eventId: number; eventDate: string; deadline: string | null; pledges: FinancialPledge[] }) {
  ...
  return <div className="space-y-5">
    ...
    {section === "send" && <section className="rounded-2xl border border-[#e7e1d7] bg-white p-4 shadow-sm sm:p-6">
      ...
      <div className="mt-5 flex flex-wrap gap-2"><Button type="button" variant="secondary" loading={busy} disabled={busy} onClick={() => void buildPreview()}>Load Recipients</Button>{preview && <><label className="flex min-h-11 items-center gap-2 rounded-xl border border-stone-300 px-3 text-sm"><input type="checkbox" checked={bulkConfirmed} onChange={(event) => setBulkConfirmed(event.target.checked)} />Confirm sending to {recipientCount} contributors ({messageCount} messages{deliveryMode === "both" ? ": WhatsApp + SMS" : ""})</label><Button type="button" variant="primary" disabled={busy || !bulkConfirmed || recipientCount === 0 || !providerReady} onClick={() => void send()}>Send Reminders</Button></>}</div>
