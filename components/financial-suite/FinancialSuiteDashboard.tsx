...
export default function FinancialSuiteDashboard({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [notice, setNotice] = useState(""); const [mode, setMode] = useState<"pledge" | "payment" | null>(null);
  ...
  async function savePayment(values: Parameters<typeof recordPayment>[1]) {
    ...
    setMode(null); setSelected(null); setNotice(`Payment recorded. Status: ${statusLabels[result.calculated_status]}.`); await load();
  }
