...
export default function FinancialSuiteDashboard({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [notice, setNotice] = useState(""); const [mode, setMode] = useState<"pledge" | "payment" | "history" | "correct-payment" | "import" | "bulk" | "reminder" | null>(null);
  ...
  return <section className="space-y-6">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><Link href="/events" className="text-sm font-semibold text-emerald-700">← Events</Link><h1 className="mt-2 text-3xl font-bold text-slate-900">Michango &amp; Ahadi</h1><p className="text-slate-600">{data?.event.title ?? "Financial Suite"} · Contributions &amp; Pledges</p></div>
      <div className="flex flex-wrap gap-2"><button onClick={() => setMode("import")} className="rounded-xl bg-blue-700 px-4 py-2 text-sm font-semibold text-white">Import Excel</button><button onClick={() => setMode("bulk")} className="rounded-xl bg-red-700 px-4 py-2 text-sm font-semibold text-white">Bulk Actions</button><button onClick={downloadPledgeTemplate} className="rounded-xl border bg-white px-4 py-2 text-sm font-semibold">Legacy template</button><button disabled={!data} onClick={() => data && exportPledges(data.event.title, data.pledges)} className="rounded-xl border bg-white px-4 py-2 text-sm font-semibold">Export Excel</button><button onClick={() => { setSelected(null); setMode("pledge"); }} className="rounded-xl bg-emerald-600 px-4 py-2 text-sm font-semibold text-white">+ Create pledge</button></div>
    </div>
