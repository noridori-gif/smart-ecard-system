...
export default function FinancialSummaryStrip({summary}:{summary:FinanceSummary}) {
  ...
  return <section aria-label={t("financial.overview")} className="grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-5">{items.map(([label,value])=><article key={label} className="sep-card min-w-0 p-4"><p className="text-[13px] font-semibold uppercase tracking-wide text-slate-500">{label}</p><p className="mt-2 truncate text-[22px] font-bold tabular-nums text-slate-950" title={value}>{value}</p></article>)}</section>;
}
