...
export default function FinancialCommunicationDialog({
  ...
  return <section aria-labelledby="communication-dialog-title" className="space-y-5">
    <div className="flex items-start justify-between gap-4"><div><h2 id="communication-dialog-title" className="text-xl font-bold">{title}</h2><p className="mt-1 text-sm text-slate-600">{kind === "reminder" ? "Send a payment reminder to this contributor?" : "Thank this contributor for completing their contribution."}</p></div><Button type="button" variant="ghost" size="sm" onClick={onClose} aria-label="Close communication dialog">Close</Button></div>
    ...
    {sendError && <p role="alert" className="rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-700">{sendError}</p>}
    <div><div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><Button type="button" variant="secondary" onClick={onClose}>Cancel</Button><Button type="button" variant="outline" loading={busy} disabled={busy} onClick={() => void preview()}>{busy ? "Loading…" : "Preview"}</Button><Button type="button" variant="primary" disabled={busy || !providerReady || previewRequired || confirmationRequired} onClick={() => void send()}>Send</Button></div>{previewError && <p role="alert" className="mt-2 rounded-xl bg-red-50 p-3 text-sm text-red-700">{previewError}</p>}{!busy && previewRequired && !previewError && <p className="mt-2 text-sm text-slate-600">{language === "sw" ? "Bonyeza Preview kwanza kuthibitisha ujumbe kabla ya kutuma." : "Preview the message before sending."}</p>}</div>
  </section>;
}
