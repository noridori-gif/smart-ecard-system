...
export default function MakeIntegrationPanel({ eventId, events }: { eventId: number | null; events: EventOption[] }) {
  ...
  return <section className="rounded-2xl border bg-white p-5 shadow-sm" aria-labelledby="make-integration-title">
    <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><div><p className="text-xs font-black uppercase tracking-wider text-emerald-700">Make.com</p><h2 id="make-integration-title" className="mt-1 text-xl font-black">{t.title}</h2></div><label className="text-sm font-bold">{t.event}<select value={effectiveEventId ?? ""} onChange={(event) => chooseEvent(event.target.value)} className="mt-1 min-h-11 w-full rounded-xl border bg-white px-3 font-normal sm:w-64"><option value="">{t.chooseEvent}</option>{events.map((event) => <option key={event.id} value={event.id}>{event.title}</option>)}</select></label></div>
