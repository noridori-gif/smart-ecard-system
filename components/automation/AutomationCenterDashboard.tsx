...
const healthCopy = {
  en: { processorStopped: "Workflow processor has not picked up these events.", queued: "Queued normally", waiting: "Waiting for scheduled time", failed: "Failed after attempts" },
  sw: { processorStopped: "Kichakataji cha workflow hakijachukua matukio haya.", queued: "Yapo kwenye foleni kawaida", waiting: "Yanasubiri muda uliopangwa", failed: "Yameshindwa baada ya majaribio" },
} as const;
...
export default function AutomationCenterDashboard() {
  ...
  const processorStalled = workflows.filter((item) => item.status === "pending" && item.attempt_count === 0 && new Date(item.available_at).getTime() <= observedAt && observedAt - new Date(item.created_at).getTime() > 10 * 60_000).length;
  const waitingForTime = workflows.filter((item) => item.status === "pending" && new Date(item.available_at).getTime() > observedAt).length;
  const queuedNormally = workflows.filter((item) => item.status === "pending" && new Date(item.available_at).getTime() <= observedAt && !(item.attempt_count === 0 && observedAt - new Date(item.created_at).getTime() > 10 * 60_000)).length;
  const unhealthy = Boolean(data?.sourceErrors.length || counts.failed || stale || processorStalled);
  const health = healthCopy[language];
  ...
  return <div className="space-y-6">
    ...
    <section className={`rounded-2xl border p-5 ${unhealthy ? "border-amber-200 bg-amber-50" : "border-emerald-200 bg-emerald-50"}`}><div className="flex items-center justify-between gap-4"><div><h2 className="text-lg font-black">{t.health}</h2><p className="mt-1 text-sm">{unhealthy ? t.degraded : t.healthy} · {data?.sourceErrors.length ? `${data.sourceErrors.join(", ")} unavailable` : t.queueClear}</p>{processorStalled > 0 && <p className="mt-2 font-bold text-amber-900">{health.processorStopped} ({processorStalled})</p>}<div className="mt-3 flex flex-wrap gap-2 text-xs"><span className="rounded-full bg-white/70 px-3 py-1">{health.queued}: {queuedNormally}</span><span className="rounded-full bg-white/70 px-3 py-1">{health.waiting}: {waitingForTime}</span><span className="rounded-full bg-white/70 px-3 py-1">{health.failed}: {counts.failed}</span></div></div><span className={`size-4 rounded-full ${unhealthy ? "bg-amber-500" : "bg-emerald-500"}`} /></div></section>
    ...
    <MakeIntegrationPanel eventId={eventId === "all" ? null : Number(eventId)} events={data?.events ?? []} />
