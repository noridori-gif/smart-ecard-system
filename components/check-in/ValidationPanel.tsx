...
export default function ValidationPanel({ result, errorMessage, checking, onNext }: { result: CheckInResult | null; errorMessage: string; checking: boolean; onNext: () => void }) {
  ...
  return <section aria-labelledby="validation-title">
    ...
    <div aria-live="polite" aria-atomic="true" className={`mt-4 min-h-40 rounded-2xl border p-5 shadow-sm sm:p-6 ${checking ? "border-emerald-200 bg-emerald-50" : state ? config.shell : "border-[#e7e1d7] bg-white"}`}>
      {checking ? <div className="flex min-h-28 items-center justify-center gap-3 text-emerald-700"><span className="h-5 w-5 animate-spin rounded-full border-2 border-current border-r-transparent motion-reduce:animate-none" /><b>Verifying Event Pass…</b></div>
