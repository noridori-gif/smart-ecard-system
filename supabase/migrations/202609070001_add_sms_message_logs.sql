-- SMS sends were fire-and-forget (app/api/sms/send-invitation/route.ts just
-- logged to console) -- there was no persisted record of whether a guest was
-- ever sent an SMS, unlike WhatsApp (whatsapp_message_logs). This mirrors
-- that table's shape for SMS so the guest list can show a real sent-status
-- per channel. No delivered_at/read_at columns: Beem's API response is an
-- immediate accept/reject with no async delivery webhook wired into this
-- app (unlike Meta's WhatsApp webhook), so there is no later status update
-- to store.
...
create index if not exists sms_message_logs_invitation_id_idx
  on public.sms_message_logs (invitation_id);

-- Extend the permanent-deletion preview/cleanup functions the same way
-- 202608100001 did for whatsapp_message_logs -- forward-only, not applied
-- automatically, matching that migration's own convention.
...
begin
  ...
  return jsonb_build_object(
    ...
    'smsMessageLogs', (select count(*) from public.sms_message_logs log
      join public.invitations invitation on invitation.id=log.invitation_id
      where invitation.event_id=target.id),
...
begin
  ...
  delete from public.sms_message_logs where invitation_id in (select id from public.invitations where event_id=target_event_id);
